    ...
    def process_point_value(self, command_type, command, index, op_type):
        ...
        try:
            ...
            if 'Control' in str(command):
                _log.debug(
                    "command_code={},command_code={},command_ontime={}".format(command.status, command.functionCode,
                                                                               command.onTimeMS))
                for point in self.outstation.get_agent().point_definitions.all_points():
                    if point.name in str(
                            point_value.point_def) and point.index == index and point.attribute == 'ShuntCompensator.sections':
                        if 'ON' in str(command.functionCode):
                            ...
                            print(json.dumps(msg))
                    if point.name in str(
                            point_value.point_def) and point.index == index and point.attribute == 'Switch.open':
                        if 'ON' in str(command.functionCode):
                            self._diff.clear()
                            self._diff.add_difference(point.measurement_id, point.attribute, 0, 1)
                            msg = self._diff.get_message()
                            ...
                            print(json.dumps(msg))
                    if point.name in str(
                            point_value.point_def) and point.index == index and point.attribute == 'RegulatingControl.Mode':
                        if 'ON' in str(command.functionCode):
                            self._diff.clear()
                            self._diff.add_difference(point.measurement_id, point.attribute, 1, 0)
                            msg = self._diff.get_message()
                ...
                for point in self.outstation.get_agent().point_definitions.all_points():
                    if point.name in str(point_value.point_def) and point.index == index:
                        self._diff.clear()
                        self._diff.add_difference(point.measurement_id, point.attribute, command.value,
                                                  0)  # value : received value
                        msg = self._diff.get_message()
                        self._gapps.send(self._publish_to_topic, json.dumps(msg))
                        print(json.dumps(msg))
...
def on_message(headers, message):
    simulation_id = headers["destination"]
    if (dnp3_object_list.__len__() > 0):
        updates = dnp3_object_list[0].create_message_updates(headers, message)

        print("Outstation Updates Created")

        for cimMapping in dnp3_object_list:
            print(dnp3_object_list)
